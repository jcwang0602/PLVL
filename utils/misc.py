...
def collate_fn(raw_batch):
    ...
    if len(raw_batch) == 7:
        label = torch.stack(raw_batch[5])
        # idx = raw_batch[6]
        # sam_mask = torch.stack(raw_batch[6])
        batch = [img_data, text_data, bbox, label, raw_batch[6]]
    elif len(raw_batch) == 9:
        label = torch.stack(raw_batch[5])
        stage1_label = torch.stack(raw_batch[6])
        stage2_label = torch.stack(raw_batch[7])
        stage3_label = torch.stack(raw_batch[8])
        batch = [img_data, text_data, bbox, label, stage1_label, stage2_label, stage3_label]
...
def collate_fn_vis(raw_batch):
    ...
    label = torch.stack(raw_batch[5])
    batch = [img_data, text_data, bbox, label, raw_batch[-1]]
